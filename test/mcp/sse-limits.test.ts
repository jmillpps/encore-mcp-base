...
test("legacy SSE rejects streams beyond the configured connection limit", async (t) => {
  ...
  await expectOAuthError(
    await fetch(`${service.origin}/sse`, { headers: { accept: "text/event-stream", origin: "https://chatgpt.com" } }),
    429,
    "rate_limited",
  );
});
...
test("Streamable HTTP SSE rejects streams beyond the configured connection limit", async (t) => {
  ...
  const controller = new AbortController();
  t.after(() => controller.abort());
  const stream = await getMcpStream(service.origin, sessionId, controller.signal);
  assert.equal(stream.status, 200);
  assert.ok(stream.body);
  await assertSseOpen(new SseReader(stream.body.getReader()));
  await expectOAuthError(await getMcpStream(service.origin, sessionId), 429, "rate_limited");
});

function getMcpStream(origin: string, sessionId: string, signal?: AbortSignal): Promise<Response> {
